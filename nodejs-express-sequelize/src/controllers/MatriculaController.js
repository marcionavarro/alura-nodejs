...
class MatriculaController extends Controller {
  ...
  async pegaMatriculasPorEstudantes(req, res) {
    const { estudante_id } = req.params;
    try {
      const listaMatriculasPorEstudante = await matriculaServices.pegaEContaERegistros({
        where: {
  ...
  async pegaCursosLotados (req, res) {
    ...
    try {
      const cursosLotados = await matriculaServices.pegaEContaERegistros({
        where: {
          status: 'matriculado'
        },
        attributes: ['curso_id'],
